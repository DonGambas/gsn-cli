...
class GsnTestEnvironmentClass {
  ...
  async deployGsn(host: string): Promise<GSNContractsDeployment> {
    ...
    if (_host == null) {
      ...
      throw new Error(
        `startGsn: expected network (${supportedNetworks().join("|")}) or url`
      );
    }
    ...
    if (deploymentResult.paymasterAddress != null) {
      const balance = await commandsLogic.fundPaymaster(
        from,
        deploymentResult.paymasterAddress,
        ether("1")
      );
      console.log(
        "Naive Paymaster successfully funded, balance:",
   ...
   * @return
   */
  async startGsn(host: string, port: number): Promise<TestEnvironment> {
    await this.stopGsn();
    const _host: string = getNetworkUrl(host);
    if (_host == null) {
      // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
  ...
  async _runServer(
    ...
    const transactionManager = new TransactionManager(
      ...
      configureServer(relayServerParams)
    );
    ...
    this.httpServer = new HttpServer(port, logger, backend);
  ...
  async loadDeployment(
    ...
    if (
      tokenAddress != null &&
      !isSameAddress(tokenAddress, constants.ZERO_ADDRESS)
    ) {
      const code = await contractInteractor.getCode(tokenAddress);
      if (code.length <= 2) {
        throw new Error(
          `No contract deployed for ERC-20 ManagerStakeTokenAddress at ${tokenAddress}`
        );
      }
    }
    return deployment;
