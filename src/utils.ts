...
export function getKeystorePath(keystorePath: string): string {
  ...
  if (
    fs.lstatSync(keystorePath).isDirectory() &&
    fs.existsSync(keystorePath + "/keystore")
  ) {
    return keystorePath;
  } else if (
    fs.lstatSync(keystorePath).isFile() &&
    path.basename(keystorePath) === "keystore"
  ) {
...
export function getServerConfig(configFilename: string): ServerConfigParams {
  if (
    !fs.existsSync(configFilename) ||
    !fs.lstatSync(configFilename).isFile()
  ) {
...
export function saveDeployment(
  ...
  saveContractToFile(
    deploymentResult.stakeManagerAddress,
    workdir,
    "StakeManager.json"
  );
  saveContractToFile(
  ...
  saveContractToFile(
    deploymentResult.forwarderAddress,
    workdir,
    "Forwarder.json"
  );
  saveContractToFile(
    deploymentResult.relayRegistrarAddress,
    workdir,
    "RelayRegistrar.json"
  );
  saveContractToFile(
    deploymentResult.managerStakeTokenAddress,
    workdir,
    "ManagerStakeTokenAddress.json"
  );
}
...
export function showDeployment(
  ...
  Paymaster ${paymasterTitle != null ? "(" + paymasterTitle + ")" : ""}: ${
    deploymentResult.paymasterAddress
  }`);
}
  ...
  options.forEach((option) => {
    switch (option) {
      case "n":
        commander.option(
          "-n, --network <url|name>",
          "network name or URL to an Ethereum node",
          "localhost"
        );
        break;
      case "f":
      ...
      case "h":
        commander.option(
          "-h, --hub <address>",
          "address of the hub contract (default: the address from build/gsn/RelayHub.json if exists)"
        );
        break;
      case "m":
        commander.option(
          "-m, --mnemonic <mnemonic>",
          "mnemonic file to generate private key for account 'from'"
        );
        commander.option(
          "--derivationPath <string>",
          "derivation path for the mnemonic to use, defaults to m/44'/60'/0'/0/"
        );
        commander.option(
          "--derivationIndex <string>",
          "derivation index to use with a given mnemonic, defaults to 0",
          "0"
        );
        commander.option(
          "--privateKeyHex <string>",
          "private key to use directly without mnemonic"
        );
        break;
      case "g":
        commander.option(
          "-g, --gasPrice <number>",
          "gas price to give to the transaction, in gwei."
        );
        break;
      case "l":
        commander.option(
          "-l, --gasLimit <number>",
          "gas limit to give to all transactions",
          "5000000"
        );
        break;
    }
